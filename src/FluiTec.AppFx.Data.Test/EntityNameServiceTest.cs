...
namespace FluiTec.AppFx.Data.Test
{
    ...
    public class EntityNameServiceTest
    {
        ...
        {
            Assert.IsTrue(!string.IsNullOrWhiteSpace(NameService.NameByType(typeof(DummyEntity))) &&
                          NameService.NameByType(typeof(DummyEntity)) == nameof(DummyEntity));
        }

        [TestMethod]
        public virtual void ProvidesNameForAttributedEntity()
        {
            Assert.IsTrue(!string.IsNullOrWhiteSpace(NameService.NameByType(typeof(AttributedDummyEntity))) &&
                          NameService.NameByType(typeof(AttributedDummyEntity)) == "TestEntity");
        }
    }
}