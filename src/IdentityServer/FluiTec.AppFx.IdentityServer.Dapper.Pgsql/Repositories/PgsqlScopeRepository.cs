...
namespace FluiTec.AppFx.IdentityServer.Dapper.Pgsql.Repositories
{
    ...
    public class PgsqlScopeRepository : ScopeRepository
    {
        ...
        {
            var command =
                $"SELECT {SqlBuilder.Adapter.RenderPropertyList(SqlCache.TypePropertiesChache(typeof(ScopeEntity)).ToArray())} FROM {TableName} WHERE \"{nameof(ScopeEntity.Id)}\" = ANY(@Ids)";
