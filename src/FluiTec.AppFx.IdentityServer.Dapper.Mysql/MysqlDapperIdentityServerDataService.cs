...
namespace FluiTec.AppFx.IdentityServer.Dapper.Mysql
{
	...
	public class MysqlDapperIdentityServerDataService : MysqlDapperDataService, IIdentityServerDataService
	{
		...
		protected virtual void RegisterIdentityRepositories()
		{
			...
			RegisterRepositoryProvider(new Func<IUnitOfWork, IClientRepository>(work => new MysqlClientRepository(work)));
			RegisterRepositoryProvider(
				new Func<IUnitOfWork, IClientScopeRepository>(work => new MysqlClientScopeRepository(work)));
			RegisterRepositoryProvider(
				new Func<IUnitOfWork, IIdentityResourceClaimRepository>(work => new MysqlIdentityResourceClaimRepository(work)));
			RegisterRepositoryProvider(
			...
			RegisterRepositoryProvider(
				new Func<IUnitOfWork, ISigningCredentialRepository>(work => new MysqlSigningCredentialRepository(work)));
