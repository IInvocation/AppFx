...
namespace FluiTec.AppFx.Identity.Dapper.Mysql.Repositories
{
    ...
    public class MysqlDapperUserRepository : DapperUserRepository
    {
        ...
        {
            var command =
                $"SELECT {SqlBuilder.Adapter.RenderPropertyList(SqlCache.TypePropertiesChache(typeof(IdentityUserEntity)).ToArray())} FROM {TableName} WHERE {nameof(IdentityUserEntity.Id)} IN @Ids";
