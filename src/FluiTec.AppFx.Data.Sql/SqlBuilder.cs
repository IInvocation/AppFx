...
namespace FluiTec.AppFx.Data.Sql
{
	...
	public class SqlBuilder
	{
		...
		public string SelectByFilter(Type type, string filterProperty, string[] selectFields = null)
		{
			...
			// try to find statement in cache and return it
			if (_selectByFilterQueries.TryGetValue(new KeyValuePair<RuntimeTypeHandle, string>(type.TypeHandle, sqlKey), out string sql)) return sql;
		...
		public List<KeyValuePair<PropertyInfo, string>> ParameterList(Type type)
		{
			...
			// generate paramList
			paramList = SqlCache.TypePropertiesChache(type).Select(p => new KeyValuePair<PropertyInfo, string>(p, Adapter.RenderParameterProperty(p)))
				.ToList();
