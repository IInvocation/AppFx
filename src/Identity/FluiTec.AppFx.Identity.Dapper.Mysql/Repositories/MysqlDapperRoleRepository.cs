...
namespace FluiTec.AppFx.Identity.Dapper.Mysql.Repositories
{
    ...
    public class MysqlDapperRoleRepository : DapperRoleRepository
    {
        ...
        {
            var command =
                $"SELECT {SqlBuilder.Adapter.RenderPropertyList(SqlCache.TypePropertiesChache(typeof(IdentityRoleEntity)).ToArray())} FROM {TableName} WHERE {nameof(IdentityUserEntity.Id)} IN @Ids";
